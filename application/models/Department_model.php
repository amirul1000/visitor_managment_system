...
class Department_model extends CI_Model
{
	 ...
	 *
     */
    function get_all_users_department(){
        $this->db->order_by('id', 'desc');
	...
	/** Get limit users-department
	 *@param $limit - limit of query , $start - start of db table index to get query
	 *
     */
    function get_limit_users_department($limit, $start){
		$this->db->order_by('id', 'desc');
        $this->db->limit($limit, $start);
		$this->db->where('users_id', $this->session->userdata('id'));
        $result = $this->db->get('department')->result_array();
    ...
    /** Count users-department rows
	 *
     */
	function get_count_users_department(){
	   $this->db->where('users_id', $this->session->userdata('id'));
       $result = $this->db->from("department")->count_all_results();
	   $db_error = $this->db->error();
		if (!empty($db_error['code'])){
